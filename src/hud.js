(function () {
  ...
  function createCoreRing(canvasId) {
    ...
    function drawPentagon(radius, color, fillAlpha) {
      ctx.save();
      ctx.translate(cx, cy);
      ctx.beginPath();
      for (let i = 0; i < 5; i++) {
    ...
    function draw() {
      ...
      // Äußere Ringe, gegenläufig rotierend
      drawRing(R, 2, 48, t * spin, 0.85, color);
      drawRing(R * 0.82, 1.5, 32, -t * spin * 1.4, 0.6, color);
      drawRing(R * 0.66, 3, 24, t * spin * 0.7, 0.4 + energy * 0.3, color);
      ...
      // Zentrales Pentagon
      drawPentagon(R * 0.42 * (0.9 + energy * 0.15), color, 0.14 + energy * 0.1);
